    ...
    <div className="w-full max-w-3xl mx-auto px-4 py-4 sm:py-6">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm relative overflow-hidden mb-5">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
          ...
          {onOpenQuickTip && (
            <button
              id="challenge-topic-quick-tip-btn"
              type="button"
              onClick={onOpenQuickTip}
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border transition shadow-2xs ${
                isQuickTipOpen
                  ? 'bg-amber-500 text-slate-950 border-amber-600 ring-2 ring-amber-500/20'
                  : 'bg-amber-50 hover:bg-amber-100 text-amber-900 border-amber-300'
              }`}
              title="Open Quick Tip rule panel"
            >
              <Lightbulb className="w-3.5 h-3.5 fill-amber-300 text-amber-700" />
              <span>Quick Tip: {question.category.replace('-', ' ')}</span>
            </button>
          )}
        </div>
